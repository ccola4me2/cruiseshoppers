...
import {
  handleListAdvisors,
  handleAdminUpdateAdvisor,
  handleSetAdvisorSpecialsPlan,
  handleSetUserStatus,
  handleDeleteUser,
  handleEmailTest,
  handleSchemaCheck,
  handleAdminClientQuotes,
  handleListClients,
  handleListAllOffers,
...
async function handleApi(request, env, ctx, path) {
  ...
  if (path === '/api/admin/advisor-update' && request.method === 'POST') return handleAdminUpdateAdvisor(request, env);
  ...
  if (path === '/api/admin/admin-update' && request.method === 'POST') return handleUpdateAdmin(request, env);
  ...
  if (path === '/api/admin/client-quotes' && request.method === 'GET') return handleAdminClientQuotes(request, env);
