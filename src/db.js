...
export async function createSpecial(db, s) {
  ...
  let cols = ['id', 'advisor_id', 'cruise_line', 'ship', 'headline', 'description', 'sail_dates',
    'rate_from', 'brochure_price', 'cabin_category', 'depart_date', 'all_dates', 'itinerary', 'expires_on', 'us_canada_only', 'status', 'created_at', 'updated_at'];
  let vals = [s.id, s.advisor_id, s.cruise_line || null, s.ship || null, s.headline, s.description || null,
    ...
    s.depart_date || null, s.all_dates ? 1 : 0, s.itinerary || null, s.expires_on || null, s.us_canada_only ? 1 : 0, 'active', now, now];
  const optional = ['expires_on', 'depart_date', 'all_dates', 'itinerary', 'cabin_category'];
...
export async function updateSpecial(db, id, advisorId, s) {
  ...
  let sets = ['cruise_line', 'ship', 'headline', 'description', 'sail_dates', 'rate_from',
    'brochure_price', 'cabin_category', 'depart_date', 'all_dates', 'itinerary', 'expires_on', 'us_canada_only', 'updated_at'];
  let vals = [s.cruise_line || null, s.ship || null, s.headline, s.description || null, s.sail_dates || null,
    ...
    s.all_dates ? 1 : 0, s.itinerary || null, s.expires_on || null, s.us_canada_only ? 1 : 0, Date.now()];
  const optional = ['expires_on', 'depart_date', 'all_dates', 'itinerary', 'cabin_category'];
  for (let attempt = 0; ; attempt++) {
    try {
