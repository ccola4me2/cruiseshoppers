...
export async function createSpecial(db, s) {
  ...
  let cols = ['id', 'advisor_id', 'cruise_line', 'ship', 'headline', 'description', 'sail_dates',
    'rate_from', 'brochure_price', 'cabin_category', 'cabin_fares', 'depart_date', 'all_dates', 'itinerary', 'departure_port', 'destination', 'expires_on', 'us_canada_only', 'status', 'created_at', 'updated_at'];
  let vals = [s.id, s.advisor_id, s.cruise_line || null, s.ship || null, s.headline, s.description || null,
    s.sail_dates || null, s.rate_from || null, s.brochure_price || null, s.cabin_category || null, s.cabin_fares || null,
    s.depart_date || null, s.all_dates ? 1 : 0, s.itinerary || null, s.departure_port || null, s.destination || null, s.expires_on || null, s.us_canada_only ? 1 : 0, 'active', now, now];
  // cabin_fares first so a pre-migration DB drops only it, not other valid columns.
  const optional = ['cabin_fares', 'expires_on', 'depart_date', 'all_dates', 'itinerary', 'departure_port', 'destination', 'cabin_category'];
...
export async function updateSpecial(db, id, advisorId, s) {
  ...
  let sets = ['cruise_line', 'ship', 'headline', 'description', 'sail_dates', 'rate_from',
    'brochure_price', 'cabin_category', 'cabin_fares', 'depart_date', 'all_dates', 'itinerary', 'departure_port', 'destination', 'expires_on', 'us_canada_only', 'updated_at'];
  let vals = [s.cruise_line || null, s.ship || null, s.headline, s.description || null, s.sail_dates || null,
    s.rate_from || null, s.brochure_price || null, s.cabin_category || null, s.cabin_fares || null, s.depart_date || null,
    s.all_dates ? 1 : 0, s.itinerary || null, s.departure_port || null, s.destination || null, s.expires_on || null, s.us_canada_only ? 1 : 0, Date.now()];
  const optional = ['cabin_fares', 'expires_on', 'depart_date', 'all_dates', 'itinerary', 'departure_port', 'destination', 'cabin_category'];
  for (let attempt = 0; ; attempt++) {
    try {
...
export async function createQuoteOffer(db, o) {
  ...
  const nn = (v) => (v == null || v === '' ? null : v);
  try {
    // Most preferred: with the insurance amount (migration 0042) + quote kind.
    await db
      .prepare(
        `INSERT INTO quote_offers
           (id, quote_request_id, advisor_id, advisor_name, advisor_email, advisor_phone, advisor_hours, price, specials, additional_info, total_price, base_fare, taxes_fees, obc_amount, gratuities_included, deposit_amount, final_payment_date, cabin_fares, quote_kind, insurance_amount, status, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'submitted', ?)`
      )
      .bind(
        ...
        nn(o.quote_kind), nn(o.insurance_amount),
        now
      )
      .run();
    return { ...o, status: 'submitted', created_at: now };
  } catch (_) { /* fall through: insurance_amount/quote_kind columns not applied yet */ }
  try {
